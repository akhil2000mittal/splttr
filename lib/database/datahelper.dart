...
class DBHelper {
  ...
  _onCreate(Database db, int version) async {
    ...
    await db.execute("CREATE TABLE Groups (groupName VARCHAR PRIMARY KEY, doc VARCHAR, groupDec VARCHAR, avtar VARCHAR)");
    ...
    await db.execute("CREATE TABLE Friends(userName VARCHAR REFERENCES Users(userName) ,friend VARCHAR REFERENCES Users(userName))");
    await db.execute("CREATE TABLE Notification(userName VARCHAR,fromUser VARCHAR, type VARCHAR, details VARCHAR,FOREIGN KEY(userName) REFERENCES Users(userName) ,FOREIGN KEY(fromUser) REFERENCES Users(userName))");     
  }
